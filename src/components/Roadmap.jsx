...
export default function Roadmap() {
  const { t } = useLanguage()
  const roadmapItems = t.roadmap.phases
          ...
          {roadmapItems.map((item, index) => (
            <Reveal key={index} delay={index * 0.1}>
              <motion.div
                ...
                className={`card-bg p-6 rounded-lg border-l-4 h-full ${getStatusColor(item.status)}`}
              >
          ...
          <div className="mt-16 text-center">
            <div className="card-bg p-8 rounded-lg max-w-4xl mx-auto">
              <h3 className="text-2xl font-orbitron font-bold text-brics-gold mb-4">
                {t.roadmap.journeyTitle}
              </h3>
              <p className="text-gray-300 mb-6">
                {t.roadmap.journeyDesc}
              </p>
